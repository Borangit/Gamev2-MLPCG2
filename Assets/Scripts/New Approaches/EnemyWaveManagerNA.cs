...
public class EnemyWaveManagerNA : MonoBehaviour
{
    ...
    private newApproachGridManager gridManager;
    private newAgentScript turtle;

    private int waveCount = 0;

    public bool startWaveOnStart = false;
    public bool havePath = false;
    ...
    void Start()
    {
        ...
        gridManager = FindObjectOfType<newApproachGridManager>();
        turtle = FindObjectOfType<newAgentScript>();
        activeEnemies = new List<GameObject>();
        enemyPathIndices = new Dictionary<GameObject, int>();

    }
    public void getPathCells(){
        ...
        for (int i = activeEnemies.Count - 1; i >= 0; i--)
        {
            ...
            if (nextPathCellIndex < pathCells.Count)
            {
                enemy.transform.position = Vector3.MoveTowards(
                    ...
                    new Vector3(pathCells[nextPathCellIndex].x, 1.25f, pathCells[nextPathCellIndex].y), 
                    Time.deltaTime * 3f
                );
                if (enemy.transform.position == new Vector3(pathCells[nextPathCellIndex].x, 1.25f, pathCells[nextPathCellIndex].y))
                {
    ...
    {
        GameObject enemy = Instantiate(enemyPrefab, new Vector3(pathCells[0].x, 1.25f, pathCells[0].y), Quaternion.identity);
